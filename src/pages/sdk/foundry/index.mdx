...
```bash
# Start anvil in Tempo mode with T1 hardfork
anvil --tempo --hardfork t1

# Fork a live Tempo network for local testing
anvil --tempo --hardfork t1 --fork-url $TEMPO_RPC_URL
...
The following flags are available for Tempo-specific features:
...
You can set `TEMPO_FEE_TOKEN` environment variable instead of passing `--tempo.fee-token` on every command:
```bash
export TEMPO_FEE_TOKEN=0x20c0000000000000000000000000000000000001
```
:::

### Limitations

Ledger and Trezor wallets are not yet compatible with the `--tempo.fee-token` option.
