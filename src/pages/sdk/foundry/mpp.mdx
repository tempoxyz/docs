...
`tempo-foundry` and `foundryup -n tempo` are deprecated. Install upstream Foundry stable with `foundryup`.
:::
...
Tempo support now ships in Foundry's stable release stream:

```bash
foundryup
```

All standard Foundry commands work as before — MPP activates only when an endpoint returns `402`. At the time of writing, `foundryup` installs `v1.7.0`.
