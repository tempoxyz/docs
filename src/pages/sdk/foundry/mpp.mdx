...
description: Use Foundry tools (forge, cast, anvil, chisel) with MPP-gated RPC endpoints on Tempo — automatic 402 handling with zero config.
---
...
[Foundry](https://github.com/foundry-rs/foundry) extends Foundry with native MPP support. When an RPC endpoint returns `402 Payment Required`, Foundry automatically handles the payment challenge — no wrapper scripts, no middleware, no code changes.
...
All standard Foundry commands work — MPP activates only when an endpoint returns `402`.
