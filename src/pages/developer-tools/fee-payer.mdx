...
The API key generated in Tempo Admin is a `tp_`-prefixed fee payer token. Put the token in the sponsor URL path:

```ts
const feePayerUrl = `https://sponsor.tempo.xyz/${feePayerToken}`
```

Do not send the fee payer token as a bearer token or another request header. The hosted fee payer authenticates the key from the `tp_...` path segment.
