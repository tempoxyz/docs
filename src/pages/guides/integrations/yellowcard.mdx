...
- **Stablecoin rails** — Multiple stablecoin support
- **Regulatory compliance** — Licensed in operating countries

Note: This guide shows integration patterns for Yellow Card's off-ramp service. Tempo's native stablecoins (pathUSD, AlphaUSD, etc.) would need to be bridged or swapped before using Yellow Card's USDC/USDT rails.
...
async function sellStablecoinFlow(
  ...
  // 2. Send stablecoins to deposit address
