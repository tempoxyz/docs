...
Fee payer configuration. When provided, the relay automatically sponsors all transactions by signing `feePayerSignature` on the filled transaction. Clients can opt out per-transaction by passing `feePayer: false`.
