...
The `precedence` option controls the signing order:

- **`'fee-payer-first'`** (default) — The fee payer service handles `eth_fillTransaction`, preparing the transaction and choosing the fee token before the sender signs. Use this when you want the sponsor to fully control fee parameters.
- **`'user-first'`** — The sender signs first. The fee payer service only counter-signs the already-signed transaction (via `eth_sendRawTransaction` or `eth_sendRawTransactionSync`) when the transaction has an empty fee payer signature. Use this when the sender needs to commit to transaction details before the sponsor gets involved.
