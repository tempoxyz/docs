...
```mermaid
flowchart TD
    app["App<br/>connect · sign · send transaction"] -->|"EIP-5792 requests"| wallet
    wallet["Tempo Wallet<br/>iframe / popup UI"] <-->|"sign / verify"| passkey
    wallet -->|"delegates to"| sdk
    passkey["tempo.xyz domain-bound passkey<br/>WebAuthn credential"] -->|"credential auth"| sdk
    sdk["Accounts SDK<br/>wallet & account logic"] -->|"built on"| infra["Wagmi + Viem<br/>Ethereum client & tooling"]
```
