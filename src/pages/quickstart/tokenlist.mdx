...
If you use an AI coding agent (e.g. [Amp](https://ampcode.com), Cursor, Claude Code), paste the following prompt to have it open a PR for you:

```
Add my token to the Tempo Token List in tempoxyz/tempo-apps.
...
Add the entry to `apps/tokenlist/data/<chain_id>/tokenlist.json` and add the SVG icon
to `apps/tokenlist/data/<chain_id>/icons/<address>.svg` (lowercase address).
