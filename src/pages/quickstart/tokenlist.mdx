...
:::info[On-chain logoURI]
TIP-20 tokens can also expose an optional on-chain `logoURI` field ([TIP-1026](https://tips.sh/1026), shipped in T5) that wallets and explorers read directly from the token contract. Most issuers still register here for richer metadata extensions (`coingeckoId`, `bridgeInfo`, display `label`) and as a fallback icon for clients that don't read on-chain `logoURI`. See [Optional on-chain logoURI](/protocol/upgrades/t5#optional-on-chain-logouri).
:::
...
- Format: SVG (required for this curated registry; the on-chain `logoURI` field instead recommends rasterized PNG/WebP — see [TIP-1026](https://tips.sh/1026#recommended-formats))
