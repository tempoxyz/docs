...
Tempo is EVM-compatible — standard Ethereum transactions are supported, and most of your existing EVM integration will work. However, because Tempo has [no native gas token](/quickstart/evm-compatibility#handling-eth-native-token-balance-checks), some wallet behaviors like balance display and gas quoting need adjustment regardless of which transaction type you use.
