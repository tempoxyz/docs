...
The Tempo API provides a hosted JSON-RPC relay that fills, sponsors, and broadcasts Tempo Transactions. Authenticate with a [Tempo API key](/docs/api/authentication), then use the Relay API as your relay-enriched RPC URL:
...
Use the Relay API above for authenticated sandbox and production integrations.

<span id="use-a-fee-payer-in-production" />

## When to run your own fee payer

Run your own fee payer service when you need:
...
- Sponsorship for traffic outside Tempo's hosted policy

See [Sponsor User Fees](/docs/guide/payments/sponsor-user-fees) to set up and connect your own fee payer service.

<span id="next-steps-for-fee-sponsorship" />
