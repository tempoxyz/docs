...
`tempo download` automatically discovers the latest snapshot manifest for the selected chain, resolves the chain's default data directory, and downloads the required consensus data. You do not need to pass `--manifest-url`, `--datadir`, or `--skip-consensus=false` for the standard workflow. `--manifest-url` and `--datadir` remain available as explicit overrides.
