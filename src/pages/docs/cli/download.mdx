...
As of v1.13.2, `tempo download` automatically discovers the latest snapshot manifest for the selected chain, resolves the chain's default data directory, and downloads the required consensus data. You do not need to pass `--manifest-url`, `--datadir`, or `--skip-consensus=false` for the standard workflow. `--manifest-url` and `--datadir` remain available as explicit overrides.
...
| `--datadir <path>` | Override the chain's default data directory |
| `-u, --url <url>` | Download a single legacy snapshot archive URL |
| `--manifest-url <url>` | Override automatic discovery with a specific modular snapshot manifest URL |
