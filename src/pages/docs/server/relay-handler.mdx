...
const client = createClient({ chain: tempo, transport: http() })
const handler = Handler.relay({ getClient: () => client })
// ---cut---
...
For sponsored requests, the relay uses `feePayer.feeToken`, then the request's explicit `feeToken`, then the first candidate returned by [`resolveTokens`](#resolvetokens). It does not inspect the sender's balances because the sponsor pays the fee.
...
4. Fall back to the first candidate returned by `resolveTokens` when no candidate has a positive balance.

Set `features: 'all'` to enable fee-token resolution. Provide `resolveTokens` to add candidate tokens; self-paid fills also consider TIP-20 call targets.
...
Resolves the Viem client for a chain. The relay calls it without an argument for the default client and with the request's resolved chain ID when available.
...
const client = createClient({ chain: tempo, transport: http() })
const getClient = () => client // [!code focus]

const handler = Handler.relay({ getClient })
```

The remaining examples reuse this `getClient` callback.
...
Set this to `'all'` to enable fee-token resolution, automatic swaps, balance diffs, and fee estimates. Configure `resolveTokens` to provide fee-token candidates and `feePayer` to enable sponsorship.
...
Returns candidate token addresses for fee-token resolution. The relay reads the account's balance for each token and prefers the highest positive balance after checking the account's onchain preference. Omitting this option disables configured candidate resolution.
