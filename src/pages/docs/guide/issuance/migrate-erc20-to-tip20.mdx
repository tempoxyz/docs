...
| Lifecycle manager | Issuance has subscriptions, redemptions, caps, or oracle pricing | Keep business logic outside TIP-20, grant the manager scoped `ISSUER_ROLE` |
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    Factory-->>Admin: Grant token admin role
    ...
    opt Compliance required
        Note over PolicyAdmin,Token: One-time policy setup and attachment
...
| Bridge operator | Bridge adapter with scoped `ISSUER_ROLE` |
| Lifecycle manager | Manager contract with scoped `ISSUER_ROLE` |
...
|---|---|
| Simple whitelist | The same allowlist applies to all token movement checks |
| Simple blacklist | The same blocklist applies to all token movement checks |
