...
| `ERC20Burnable` | `burn` and often `burnFrom` | An `ISSUER_ROLE` holder can burn only its own TIP-20 balance; use pull-then-burn for user funds |
...
## 7. Handle burn semantics

OpenZeppelin-style ERC-20 tokens may expose:
...
An `ISSUER_ROLE` holder can burn only its own TIP-20 balance. TIP-20 does not give the issuer a general `burnFrom(user, amount)` path. If your current flow uses `burnFrom`, replace it with a manager-mediated pull-then-burn flow.

```text
transferFrom(user, managerOrBridgeAdapter, amount)
managerOrBridgeAdapter burns its own TIP-20 balance
```

The pull and burn can be atomic in one adapter transaction, but the adapter temporarily takes custody of the tokens between the two calls. Confirm that this custody model is acceptable for the issuer's legal and regulatory requirements.

In practice, `managerOrBridgeAdapter` means:

| Flow | Contract that pulls and burns |
|---|---|
| Native redemption | Lifecycle manager |
| Bridge out of Tempo | Bridge adapter |
| Migration burn | Migration adapter or issuer-controlled burn contract |
| Simple issuer operation | Issuer wallet for admin-only mint/burn operations, not user-facing redemption |

For bridge or redemption flows, the lifecycle manager or bridge adapter should hold `ISSUER_ROLE`.

Do not treat `BURN_BLOCKED_ROLE` as a general redemption role. Use it only for blocked-account or compliance recovery flows.
...
## 9. Move custom logic outside the token
...
## 10. Map compliance hooks to TIP-403
...
A TIP-20 token points to one `transferPolicyId`. Do not model compliance as attaching both an allowlist policy and a blocklist policy directly to the token. Use a simple whitelist or blacklist when one rule applies across token movement. Use a compound policy when sender, recipient, and mint-recipient rules need to differ.
...
### Handle burn and redemption

Normal burn and redemption are not a separate TIP-403 policy attachment.

For normal redemption:

```text
user approves lifecycle manager or bridge adapter
manager pulls tokens with transferFrom
manager burns its own TIP-20 balance using ISSUER_ROLE
```

For blocked-account recovery:

```text
BURN_BLOCKED_ROLE can use burnBlocked(...)
```

`burnBlocked` is a special compliance path. It should not be treated as the normal redemption or bridge burn path.
...
## 11. Avoid incompatible ERC-20 assumptions
...
- Do not map `burnFrom` directly. Use a lifecycle manager or bridge adapter that pulls tokens with `transferFrom`, then burns its own TIP-20 balance.
- Do not ignore 18-to-6 decimal conversion.
- Do not put compliance or subscription logic inside the token.

## 12. Consult the operational reference
