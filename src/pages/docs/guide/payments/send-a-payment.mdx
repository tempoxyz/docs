...
<Tabs stateKey="library">
  <Tab title="Viem">
    ...
    const calls = recipients.map((to) => ({ // [!code hl]
      to: token, // [!code hl]
    ...
    const receipt = await client.sendTransactionSync({ calls }) // [!code hl]
    console.log(receipt.transactionHash)
    ```

    ```ts twoslash [viem.config.ts]
    // [!include ~/snippets/viem.config.ts:moderato]
    ```
...
`sendTransactionSync` puts every call in one all-or-nothing Tempo transaction and returns its receipt. `sendCalls` is for wallet requests; a loop or `Promise.all` sends separate transactions.
