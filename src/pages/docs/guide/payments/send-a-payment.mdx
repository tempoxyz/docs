...
Display a link to the transaction receipt after submission succeeds. Transaction success alone does not confirm delivery: a [receive policy](/docs/guide/payments/configure-receive-policies) can redirect funds to `ReceivePolicyGuard` instead of the receiver. Before showing a payment as delivered, [verify its transfer events](#confirm-delivery-not-just-transaction-success).
...
For held payments, also index `TransferBlocked` from `ReceivePolicyGuard` (`0xB10C000000000000000000000000000000000000`). Filter by the payment's token and receiver, and save the `receipt` bytes for the authorized claimer. See [Monitor blocked receipts](/docs/guide/payments/configure-receive-policies#monitor-blocked-receipts).

## Stablecoin payment best practices

### Confirm delivery, not just transaction success

Match events from the expected token contract to the payment's sender, recipient, and amount. Do not rely on the first `Transfer` log in a transaction: a receipt can contain multiple transfers.

- **Failed**: the transaction reverted; the payment was not delivered. Transaction fees may still apply.
- **Delivered**: the matching `Transfer` credits the intended receiver. For a [virtual address](/docs/protocol/tip20/virtual-addresses#what-this-changes-for-wallets-explorers-and-indexers), verify the forwarding transfer to its master wallet and count the sequence as one payment.
- **Held**: the transfer credits `ReceivePolicyGuard`, with a corresponding `TransferBlocked` event. The receiver has not been paid; retain the blocked receipt for [recovery](/docs/guide/payments/configure-receive-policies#claim-blocked-receipts).

If no matching events are found, leave delivery unconfirmed. A successful transaction alone is not enough to mark a payment, withdrawal, or deposit as delivered.
