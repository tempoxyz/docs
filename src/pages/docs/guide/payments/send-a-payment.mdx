...
:::warning[T6 testnet behavior — confirm delivery, not just transaction success]
On T6 networks, a blocked TIP-20 `transfer` / `transferFrom` still succeeds, but credits `ReceivePolicyGuard` at `0xB10C000000000000000000000000000000000000` instead of the intended receiver.
...
See [account-level receive policies on the T6 upgrade page](/docs/protocol/upgrades/t6#account-level-receive-policies).
:::
