...
<Tabs stateKey="library">
  <Tab title="Viem">
    ...
    const hash = await client.sendTransaction({ calls }) // [!code hl]
    console.log(hash)
    ```
...
Passing `calls` creates one all-or-nothing Tempo transaction. `sendTransaction` returns its hash immediately; use `sendTransactionSync` when you need the receipt. Use `sendCalls` or `sendCallsSync` for connected-wallet batch requests.
