...
Receive policies let a receiver define which [TIP-20](/docs/protocol/tip20/overview) tokens it accepts and which senders may send those tokens to it. This guide summarizes the builder-facing behavior from [TIP-1028](https://tips.sh/1028).
...
Configure a receive policy when an address should reject some incoming TIP-20 tokens or some incoming senders without making the sender's TIP-20 call fail.

The specification gives two examples:
...
## Claim or burn receipts

A claim consumes one full receipt and releases the full amount to one destination. Partial claims are not supported.
...
Claim paths and burn eligibility have additional policy checks. See the [protocol overview](/docs/protocol/tip403/receive-policies#claims) before implementing recovery flows.
