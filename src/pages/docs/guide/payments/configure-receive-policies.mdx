...
If a receive policy blocks an ordinary inbound token transfer or mint, the TIP-20 call still succeeds. The funds are credited to `ReceivePolicyGuard` instead of the receiver, and `ReceivePolicyGuard` records a receipt that can be claimed later by the configured recovery authority. See [payment channels](#payment-channels-at-t12) for channel-specific handling at T12.
...
For channels opened at or after T12, configure the payee's receive policy to accept the channel token from the payer. Funding checks this policy before moving funds; a later policy change can block capture. Channels opened before T12 present the reserve as sender during capture. Refunds present the reserve as sender to the payer.

Channel receive-policy rejection reverts the operation. Adjust the policy and retry. Correlate channel events with physical token transfers to identify the payer. See [TIP-1095](https://github.com/tempoxyz/tempo/blob/main/tips/tip-1095.md).
