...
For direct token transfers and minting, if the receiver's policy blocks delivery, the funds go to `ReceivePolicyGuard` for later recovery. The transaction succeeds, and the configured recovery authority can claim the funds. From T12, a blocked [channel payment or refund](#payment-channels-at-t12) fails and leaves the funds in place.

For direct transfers and minting, the token's [TIP-403](/docs/protocol/tip403/spec) policy checks run first. If those checks fail, the transaction fails and the funds do not move.
...
For direct token transfers and minting:
...
## Payment channels at T12

Before T12, a channel payment or refund blocked by a receive policy could go to the recovery contract and still count as completed. From T12, the operation fails and the funds stay where they were. Fix the receive policy and retry.

For channels opened at or after T12, the recipient's receive policy must accept the token from the payer. Funding checks this before moving funds; a later policy change can block payment. For older channels, the policy checks the reserve—the contract holding the funds—as the sender. Refunds also use the reserve as the sender.

Use channel events alongside token transfers to identify the payer. Read the [specification](https://github.com/tempoxyz/tempo/blob/main/tips/tip-1095.md).
