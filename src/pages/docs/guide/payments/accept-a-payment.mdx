...
By default, TIP-20 payments are credited to your address automatically. You don't need to submit a transaction to accept them.

To restrict incoming tokens or senders, configure a [receive policy](/docs/guide/payments/configure-receive-policies). A payment blocked by that policy can still succeed onchain, but its funds are held in `ReceivePolicyGuard` instead of your balance. The policy determines [who can claim the held funds](/docs/guide/payments/configure-receive-policies#claim-blocked-receipts).
...
Watch the expected token contract and filter `Transfer` events by your receiving address, as shown below. For [virtual addresses](/docs/protocol/tip20/virtual-addresses#what-this-changes-for-wallets-explorers-and-indexers), verify the forwarding transfer to the master wallet and count the sequence as one payment.

Payments held by a receive policy credit `ReceivePolicyGuard`, so the recipient filter below excludes them. To track held payments separately, [index `TransferBlocked`](/docs/guide/payments/configure-receive-policies#monitor-blocked-receipts) from the guard, filtered by token and receiver, and retain its `receipt` bytes for recovery.
...
<Tabs stateKey="library">
  ...
  <Tab title="Rust">
    ...
    async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .topic2(address!("0x742d35Cc6634C0532925a3b844Bc9e7595f0bEbb").into_word()) // [!code focus]
        .watch() // [!code focus]
        .await? // [!code focus]
  ...
  <Tab title="Python">
    ...
    events = contract.events.Transfer().get_logs( # [!code focus]
        from_block="latest", # [!code focus]
        argument_filters={"to": Web3.to_checksum_address("0x742d35Cc6634C0532925a3b844Bc9e7595f0bEbb")}, # [!code focus]
    ) # [!code focus]
...
5. **Verify delivery**: Match the token, recipient, and amount to the expected payment. Transaction success alone does not prove receipt; [held payments](#listen-for-tip-20-transfer-events) must be tracked separately
