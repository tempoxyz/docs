...
:::tip[Fee Payer API]
Use the [Fee Payer API](/docs/api/fee-payer) for authenticated sandbox and production sponsorship. Run your own fee payer when you need custom sponsorship policy, accounting, or operational control.
:::
...
:::tip
Tempo provides a public testnet fee payer service at `https://sponsor.moderato.tempo.xyz` that you can use for development and testing. See [Hosted Fee Payer](/docs/api/fee-payer#public-testnet-fee-payer) for endpoint details, or follow the instructions below to run your own.
:::
...
Your relay should expose the same core JSON-RPC methods described in the [Hosted Fee Payer API reference](/docs/api/fee-payer#hosted-fee-payer-endpoints), then apply your own sponsorship policy before signing.
