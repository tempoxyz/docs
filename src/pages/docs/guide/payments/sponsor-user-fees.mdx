...
To sponsor transactions with your own infrastructure, run a JSON-RPC fee payer relay that validates incoming requests, fills Tempo Transactions, and signs approved sponsorships. The fee payer account must be funded with the stablecoin it will use for fees.

Use [`Handler.relay`](https://accounts.tempo.xyz/docs/server/handler.relay) to expose `eth_fillTransaction` and apply your sponsorship policy before signing. See the [Fee Payer API](/docs/api/fee-payer) for the viem client setup.
