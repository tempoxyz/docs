...
description: Learn how storage credits lower repeat storage costs, how Refund mode works by default, and when apps should build attribution logic.
---
...
Storage credits make repeated storage workflows cheaper. They are useful for repeat business flows like DEX orders: placing an order creates storage, canceling or filling the order clears it, and the same maker may place another order later.

At the protocol layer, a storage credit belongs to the account that owns the storage, usually the contract. It does not know which user, maker, or payer caused the storage to be cleared. If a product wants the savings to follow that user later, the product needs its own attribution logic.

T7 keeps fresh storage expensive enough to discourage state-growth spam, while letting contracts reuse value from storage they clear. By default, contracts benefit automatically. App-level attribution is only needed when the product wants to control which user receives the later savings.
...
When one of that account's storage slots changes from nonzero to zero, the account earns one storage credit. Later, when the same account changes a storage slot from zero to nonzero, one available credit can offset the creditable portion of that new storage write. How and when the credit is applied depends on the account's storage creation mode.
...
In `Refund` mode, storage creations pay upfront during execution. At the end of the transaction, the protocol looks for eligible storage creations and available credits owned by the same account, consumes matched credits, and applies the gas refund.

This default mode is usually enough when a contract simply wants lower net cost for repeated storage churn. It is not user-aware: whichever eligible storage creation happens next can consume an available credit, regardless of which user caused the earlier storage deletion.

That first-come, first-served behavior is why some products build their own attribution logic. The protocol supplies the account-level credit; the app decides whether it also needs to remember which user should benefit from that credit later.
...
## Why app-level attribution can be useful

Protocol storage credits lower the contract's cost, but the protocol only tracks the storage-owning account. It does not answer the product question of which user should receive the benefit.
...
Build app-level attribution logic when all of these are true:

1. Deleted storage is clearly attributable to one user, maker, payer, or account.
2. That same user is expected to create storage again later.
...
[TIP-1064](https://tips.sh/1064) adds StablecoinDEX-specific reusable storage accounting for order storage.

The DEX keeps a dedicated state variable, `dex_storage_credits`, to track reusable order-storage credits per user. This is app-level bookkeeping, not a second protocol balance. The protocol balance belongs to the DEX contract; `dex_storage_credits` is the DEX's record of which maker should receive the benefit.
...
The DEX benchmark shows why app-level attribution can matter. Fresh order placement stays effectively unchanged, while repeat orders from a maker who already earned DEX storage credits become much cheaper.
...
App-level attribution prevents one user's storage benefit from being spent by another user.
...
When deciding whether default storage credits are enough or whether your contract should track attribution, check:
