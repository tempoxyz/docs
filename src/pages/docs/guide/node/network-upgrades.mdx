...
|---------|------|---------|-------------|----------|
| [v1.13.2](https://github.com/tempoxyz/tempo/releases/tag/v1.13.2) | Aug 27, 2026 | Testnet + Mainnet | Recommended for operators using `tempo download` to start nodes from snapshots. Includes required consensus data by default, automatically resolves the manifest and data directory, and safely replaces consensus state with `--force`. | <Badge variant="yellow">Recommended</Badge> |
