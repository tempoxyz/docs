...
As of v1.13.2, `tempo download` automatically discovers the latest snapshot manifest, resolves the default data directory, and downloads the required consensus data. See the [`tempo download` reference](/docs/cli/download) for explicit overrides and instructions for replacing existing snapshot data.
