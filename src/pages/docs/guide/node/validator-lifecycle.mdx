...
Validator fees accumulate in the [FeeManager](/docs/protocol/fees/spec-fee-amm#key-functions) and must be claimed to reach your wallet. Use the fee-recipient address and token address in which the fees accrued:
...
Use the [signer backend](/docs/guide/node/validator-keys#validator-operator-address-custody) that matches your custody setup. Anyone can submit the claim, but the fees always go to the specified fee recipient. If you changed your fee recipient or preferred token, claim each previous recipient/token pair separately. Use `--dry-run` to inspect the target and calldata without connecting a wallet or sending a transaction.
