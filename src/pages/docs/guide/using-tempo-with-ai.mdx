...
Tempo Docs installs a read-only integration: the Tempo documentation MCP server and a skill for finding and applying current developer guidance. Wallet setup, paid requests, and transaction execution are intentionally excluded.
...
```bash
npx skills add tempoxyz/docs --skill tempo-docs
```
