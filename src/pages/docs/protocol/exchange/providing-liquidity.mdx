...
## Reading tick liquidity at T12

At T12, read tick liquidity through `getTickLevel()` or sum the remaining amounts of active orders. The stored tick `totalLiquidity` aggregate becomes stale. The getter traverses individual orders, so recheck gas budgets for this read. See [TIP-1088](https://github.com/tempoxyz/tempo/blob/main/tips/tip-1088.md).
