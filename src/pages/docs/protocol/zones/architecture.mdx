...
Every address across `allowedAccounts`, `zoneGateways`, and `sequencers` must be unique. Duplicates within a list or across lists are rejected. Validate the combined lists before submission. Empty role lists deny access when their corresponding enforcement mode is enabled.
