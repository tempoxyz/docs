...
Signatures MUST be encoded using the same format as [Tempo Transaction signatures](https://tempo.xyz/docs/protocol/transactions/spec-tempo-transaction#signature-types):
...
The precompile MUST use the same verification logic as Tempo transaction signature validation. See the [Tempo Transaction Signature Validation spec](https://tempo.xyz/docs/protocol/transactions/spec-tempo-transaction#signature-validation) for details.
