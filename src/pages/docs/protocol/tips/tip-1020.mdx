...
:::info[Updated input gas price]
[TIP-1100](https://tips.sh/1100) supersedes the input charge below with 30 gas per 32-byte word, rounded up over the full calldata including the function selector. The 6-gas input price below describes the original pre-T11 specification; signature verification gas is unchanged. [TIP-1105](https://tips.sh/1105) also requires canonical ABI encoding for precompile calls.
:::
