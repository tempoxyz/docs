...
Calls MUST use canonical ABI encoding as required by [TIP-1105](https://tips.sh/1105), with no gaps, overlapping regions, trailing bytes, or nonzero padding.
...
All calls pay the Tempo precompile input cost of `30 × ceil(calldata_length / 32)` gas on the full ABI-encoded input, including the function selector, as specified by [TIP-1100](https://tips.sh/1100). This charge is applied before dispatch and ABI decoding; insufficient gas causes out-of-gas before decoding. The verification gas per signature type is in-line with the [Tempo Transaction Signature Gas Schedule](https://tempo.xyz/developers/docs/protocol/transactions/spec-tempo-transaction#signature-verification-gas-schedule):
