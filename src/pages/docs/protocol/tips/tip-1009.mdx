...
:::info[Updated expiry window and capacity]
[TIP-1093](https://tips.sh/1093) supersedes the limits below with a 300-second expiry window and a 3,000,000-entry replay-protection buffer. The validity rule is `now < validBefore <= now + 300`, where `now` is the current block timestamp. The 30-second limits, 300,000-entry capacity, pseudocode, and boundary tests below describe the original pre-T11 specification.
:::
