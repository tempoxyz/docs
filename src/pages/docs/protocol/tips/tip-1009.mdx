...
- `MAX_EXPIRY_SECS = 300` seconds

Transactions with `validBefore <= now` or `validBefore > now + 300` are rejected. The expiry window and buffer capacity follow [TIP-1093](https://tips.sh/1093).
...
```
EXPIRING_NONCE_SET_CAPACITY = 3,000,000
```

This capacity is sized for 10,000 TPS × 300 seconds = 3,000,000 transactions, ensuring entries expire before being overwritten.

#### Algorithm

When processing an expiring nonce transaction:

1. **Validate expiry window**: Reject if `validBefore <= now` or `validBefore > now + 300`
...
7. **Advance pointer**: Write `expiringNonceRingPtr = (idx + 1) % CAPACITY`
...
function checkAndMarkExpiringNonce(
    ...
    require(validBefore > now && validBefore <= now + 300, "InvalidExpiry");
    ...
    // 7. Advance pointer
    expiringNonceRingPtr = (idx + 1) % CAPACITY;
}
```
...
- Expiring nonce data is ephemeral: entries expire within 300 seconds and are reused after expiry in a fixed-size buffer (3,000,000 entries)
...
| `handler_expiringNonceWindowTooFar` | E3 | Tx with `validBefore > now + 300s` must be rejected |
...
2. **Expiry validation**:
   ...
   - `validBefore = now + 301` → reject
   - `validBefore = now + 300` → accept
...
5. **Post-expiry replay**: Submit tx, wait for expiry, sign and submit a new transaction with a new `validBefore` (should succeed with a different transaction hash)
...
These historical benchmarks measured state savings from expiring nonces compared to 2D nonces using the original 300,000-entry buffer. They are not measurements of the current 3,000,000-entry buffer.
...
- The buffer is sized (3,000,000 entries) to guarantee entries expire before being overwritten at 10k TPS
...
The current capacity of 3,000,000 assumes:
- Maximum 10,000 TPS sustained
- 300 second expiry window
