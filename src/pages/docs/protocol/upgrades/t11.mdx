...
Replay protection remains in place, so the same signed transaction cannot execute twice. Since T11, nodes enforce the five-minute maximum. T11 does not extend an already-signed transaction's expiry: its original `validBefore` still applies.
