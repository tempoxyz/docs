...
Tempo precompile calls charge `30 × ceil(calldata_length / 32)` gas before dispatch and ABI decoding. The length includes the selector; insufficient gas halts before decoding ([TIP-1100](https://tips.sh/1100)).

Duplicate checks on AccountKeychain call scopes and ZoneFactory role lists charge an additional 20 gas per value processed, before sorting ([TIP-1105](https://tips.sh/1105)).

Calls require canonical ABI encoding, without gaps, overlaps, trailing bytes, or nonzero padding. Estimate gas on the target network, especially for large inputs or fixed gas limits.
