...
## Abstract
TIP-20 tokens are a suite of precompiles that provide a built-in optimized token implementation in the core protocol. They extend the ERC-20 token standard with built-in functionality like memo fields, legacy rewards behavior, and transfer policies.
...
## Legacy Token Rewards Distribution
TIP-20 rewards are deprecated after T7. New reward opt-ins and distributions are no-ops, while already-accrued rewards remain claimable. See [rewards distribution](/docs/protocol/tip20-rewards/spec) for the legacy behavior and claim path.
