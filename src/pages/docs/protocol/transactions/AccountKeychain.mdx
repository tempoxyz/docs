...
`authorizeKey(...)` takes a `KeyRestrictions` tuple that bundles expiry, spending limits, and call scopes. `authorizeAdminKey(...)` provisions an admin key for account key management. Access-key-signed transactions cannot use a top-level `Create` call.
...
- **No Top-Level Creation**: Access-key-signed transactions cannot use a top-level `Create` call. Contracts they call may still use `CREATE` or `CREATE2`.
...
- Access-key-signed transactions cannot use a top-level `Create` call. Contract creation inside a called contract is not rejected solely because it uses `CREATE` or `CREATE2`.
...
3. Transaction executes with spending limit enforcement via internal `verify_and_update_spending()` and call-scope enforcement. A top-level `Create` call is rejected.
