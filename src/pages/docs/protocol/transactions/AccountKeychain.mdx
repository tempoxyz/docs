...
:::warning
Authorize a deny-all key as an explicitly restricted key. An authorization that sets an empty
scope list but omits limits is indistinguishable from an admin key authorization, and the node
rejects it.

Admin keys carry no limits, scopes, or expiry ([TIP-1049](https://tips.sh/1049)), so omitting
limits matches the admin shape. The node then expects the authorization to be admin-signed and
bound to the parent account, and validation fails with
`keychain validation failed: admin-signed key authorization account mismatch`.

Set both fields, so the authorization is unambiguously a limited key permitted to call nothing:
