...
- Policy-configuration functions that accept literal member addresses reject [TIP-1022](/protocol/tips/tip-1022) virtual addresses.
- TIP-20 policy checks for virtual-address transfers and mints run against the resolved master wallet, not the forwarding alias, so policy membership must be configured on the master address.
