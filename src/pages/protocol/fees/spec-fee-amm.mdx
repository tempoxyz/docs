...
:::info[Coming with T5]
The [T5 network upgrade](/protocol/upgrades/t5) introduces multihop FeeAMM routing ([TIP-1033](https://github.com/tempoxyz/tempo/blob/main/tips/tip-1033.md)). Validators can support payout tokens that don't have direct liquidity for every fee-token / payout-token pair — one fee conversion can route through two pools instead of one. The fixed-rate swap mechanism described below is unchanged for direct pairs; the new behavior is purely additive at the routing layer. FeeAMM LPs and rebalancers should be aware that one fee conversion may now reserve and consume liquidity from two pools.
:::
