...
pathUSD is issued by [Bridge](https://www.bridge.xyz) and is backed 1:1 by USD-denominated reserves. pathUSD can be minted by depositing USDC and redeemed back to USDC through Bridge.

### Why pathUSD?

Tempo is designed to be neutral across stablecoin issuers. Rather than defaulting to any single issuer's stablecoin as the quote token, pathUSD provides a neutral option that any token can pair against on the native exchange. This ensures no single stablecoin issuer has a privileged position in Tempo's liquidity graph.
