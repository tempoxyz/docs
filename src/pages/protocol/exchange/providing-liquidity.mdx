...
- `flipTick` - The price where the order will flip to when filled
  - May equal `tick`
  - Otherwise, commonly greater than `tick` if `isBid` is true
  - Otherwise, commonly less than `tick` if `isBid` is false
...
## T5 integration notes

:::info[Migration appendix]
This section captures post-T5 integration details for DEX integrators. See the [T5 network upgrade](/protocol/upgrades/t5) page for the upgrade summary.
:::
...
- If you support historical pre-T5 data and current post-T5 data, branch on the network version or activation timestamp
