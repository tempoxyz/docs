...
For `key_authorization`, a non-expiring key is encoded by omitting `expiry` (`None` / empty RLP field), not by encoding `0`.

Do not hand-encode `expiry = 0` or rely on `Some(0)`. The protocol only treats omitted expiry as "never expires," and translates that omission to `u64::MAX` when materializing the `AccountKeychain.authorizeKey(...)` call.
:::
