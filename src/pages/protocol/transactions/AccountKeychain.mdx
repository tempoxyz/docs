...
At T2, the main focus is expiry and spending limits. T3 extends the same model to cover recurring budgets and explicit call scoping, which makes the precompile a better fit for subscriptions, connected apps, and session-key-style flows.
...
- `TokenLimit` now includes `period`, so limits can be one-time or recurring.
...
Use the correct `SignatureType` for the access key you are authorizing:

- `0` = `Secp256k1`
- `1` = `P256`
- `2` = `WebAuthn`

If the access key will be used with passkey or WebAuthn signatures, use `2`. `1` is only for raw P256 signatures. Authorizing a passkey key as `P256` can later fail with a signature-type mismatch when the key is used.

For a step-by-step verification flow using `cast`, see the [Foundry guide](/sdk/foundry).
