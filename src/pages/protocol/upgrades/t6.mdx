...
T6 gives partners more control over account safety and user key management. It adds account-level receive policies so wallets, fintechs, exchanges, custodians, and orchestrators can prevent unwanted TIP-20 deposits before they reach a user account, and it adds admin access keys so users and apps can manage passkeys and delegated keys without relying on the root wallet for every change.
...
This is especially useful for accountable receivers: institutions, exchanges, custodians, on/off-ramps, embedded-wallet providers, and payout systems that need to know whether an inbound transfer was credited, redirected for review, or failed.

## Features

### Account-level receive policies

Receive policies let an account decide which TIP-20 tokens it accepts and which senders may send those tokens. This is useful for partner wallets, exchanges, custodians, on/off-ramps, payment processors, and treasury systems that need to keep unsupported or unwanted assets out of user balances.
...
- Payout systems can treat redirected funds as a first-class delivery state instead of assuming a successful transfer means the receiver was credited.
- Support teams can surface blocked transfers for recovery instead of treating every mismatch as a lost deposit.

For users and operators, the journey is simple: set the receive policy, monitor whether each inbound transfer was credited or redirected, then claim or reroute redirected funds through the recovery path.
