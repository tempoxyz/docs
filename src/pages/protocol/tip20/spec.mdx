...
:::info[Migration appendix]
This section captures TIP-20 changes shipping with the [T6 network upgrade](/protocol/upgrades/t6). Until T6 activates on testnet and mainnet, the spec above remains canonical. After T6 activation, the items below will be folded into the body and this appendix will be removed.
:::

T6 introduces two changes that affect the TIP-20 surface:
...
If the receive policy blocks the operation, **the call still succeeds**. Delivery is redirected to a new `ReceivePolicyGuard` precompile at `RECEIVE_POLICY_GUARD_ADDRESS`, which records a receipt for the transfer or mint. The originator or a recovery address designated by the receiver can later claim the recorded amount from the guard.

Notes on the TIP-20 surface:
...
- The host TIP-20 emits its standard `Transfer` event to `RECEIVE_POLICY_GUARD_ADDRESS` on a redirected transfer; the guard separately emits a `TransferBlocked` event with the information needed to claim the receipt. Blocked receipts are not enumerable on chain — indexers must subscribe to `TransferBlocked` to surface claimable funds.
...
### TIP-1065: packed user-state slot

Each value in the TIP-20 `balance` mapping is reinterpreted as a packed user-state word:
...
`balanceOf(account)` returns only the low 128-bit `balance`. The packed cache fields are not exposed through any external interface. Total supply is already capped at `uint128::MAX`, so every valid TIP-20 balance fits in the low 128 bits and no migration is required — existing slots default to their previous balance with `rewardFlag == Uninitialized`.

:::warning[Action required for partners reading raw TIP-20 storage]
Anything reading TIP-20 balance slots directly via `eth_getStorageAt` (market makers, indexers, custody reconciliation, snapshot tools) MUST mask the low 128 bits of the slot to recover the balance after T6:

```ts
balance = BigInt(slot) & ((1n << 128n) - 1n)
```

Tools that read raw storage across the T6 activation boundary without masking will see inflated balances once `rewardFlag` gets populated. The supported, version-stable read path is `balanceOf(account)` and the standard TIP-20 events — code that uses those interfaces is unaffected.
:::

`rewardRecipient(account)` remains the source of truth for reward opt-in; the cached `rewardFlag` MUST always agree with it. `setRewardRecipient(recipient)` updates both. An `Uninitialized` cache value means the implementation reads `rewardRecipient(account)` and derives effective reward state from it. Once initialized, `rewardFlag` never returns to `Uninitialized`.

Storage accounting for the packed slot follows the slot's actual zero/nonzero transition under [TIP-1000](https://tips.sh/1000). In particular, creating a cache-only slot (balance becomes zero but a cache field stays nonzero) is a nonzero-to-nonzero write and does not receive a nonzero-to-zero refund.

See [TIP-1065](https://tips.sh/1065) for the full specification, including transfer storage accounting and the rules for future cache fields in the reserved bits.
