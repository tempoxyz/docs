...
:::warning[Breaking change with T6 — raw TIP-20 storage reads]
After the [T6 network upgrade](/protocol/upgrades/t6), each value in the TIP-20 `balance` mapping is reinterpreted as a packed user-state word (low 128 bits = balance, upper 128 bits = cache fields) per [TIP-1065](https://tips.sh/1065). `balanceOf(account)` and the standard TIP-20 events are **unchanged** — code that uses them is unaffected. Tooling that reads balance slots directly via `eth_getStorageAt` (snapshot tools, market makers, custody reconciliation) MUST mask the low 128 bits after T6:

```ts
balance = BigInt(slot) & ((1n << 128n) - 1n)
```

See the [T5 → T6 migration appendix on the TIP-20 spec](/protocol/tip20/spec#t5--t6-migration) for the full storage layout.
:::
