...
description: Understand how TIP-20 virtual addresses work, why they remove sweep transactions, and how to think about deposit attribution on Tempo.
---
...
Virtual addresses are scheduled for T3. They are not active on Moderato or Presto yet.
:::

## Why this feature exists

Without virtual addresses, per-customer deposit addresses are operationally expensive. Each deposit address becomes a real onchain balance holder. Funds land there first, and then the operator has to sweep those funds back into a central wallet.
...
- no separate state is created for each deposit address
...
3. watch TIP-20 events and map the `userTag` back to the customer record
...
### It does not make all protocols virtual-address aware

Some protocols record ownership against the literal address they are given. If they mint LP shares, receipts, or similar positions to a virtual address, those positions can become stranded unless that protocol explicitly supports resolution. The Fee AMM is one example of this edge case.

### It does not bypass TIP-403 or TIP-1015

Policy checks apply to the resolved master wallet. If the master is not allowed to receive a token, deposits to that master's virtual addresses will fail too.
...
If you want implementation details, the guide covers the full operator workflow.

## Learn more

<Cards>
  <Card
    title="Guide: Use virtual addresses for deposits"
    description="Register a master wallet, derive customer deposit addresses offchain, and reconcile deposits from events."
    to="/guide/payments/virtual-addresses"
    icon="lucide:route"
  />
  <Card
    title="TIP-1022 specification"
    description="Read the full TIP with transfer-path semantics, event ordering, and invariants."
