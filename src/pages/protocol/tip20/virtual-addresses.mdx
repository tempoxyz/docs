...
A virtual address is a forwarding alias, not a balance-holding account. Each deposit emits two `Transfer` events in one transaction:

1. `Transfer(sender, virtualAddress, amount)` — attribution hop
2. `Transfer(virtualAddress, masterAddress, amount)` — forwarding hop

Collapse these into one logical deposit. Treating them independently will double-count transfers and show misleading balances. Use the `userTag` (trailing 6 bytes of the virtual address) for deposit attribution, not `balanceOf(virtualAddress)` which is always zero.

Wallets and explorers should display enough of the address to distinguish the `masterId` and `userTag`.
