...
- **Explorers and indexers need to handle the two-hop `Transfer` pattern.** Each virtual-address deposit emits two `Transfer` events in a single transaction: `Transfer(sender, virtualAddress, amount)` followed by `Transfer(virtualAddress, masterAddress, amount)`. Indexers must deduplicate these to avoid double-counting the transfer amount. The first event carries the virtual address for deposit attribution; the second shows the actual balance credit to the master.
