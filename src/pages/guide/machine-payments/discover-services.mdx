...
Call the target service directly with an MPP-capable client such as [`tempo request`](/cli/request). The service returns a `402` challenge if payment is required, and the client pays and retries.
