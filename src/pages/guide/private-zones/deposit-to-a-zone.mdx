...
The demo below supports both the standard deposit flow and the upstream `Actions.zone.encryptedDepositSync(...)` flow when you want the zone recipient and memo to stay encrypted from everyone except the sequencer.
...
const { receipt } = await Actions.zone.encryptedDepositSync(rootClient, {
  account: rootClient.account,
  amount: depositAmount,
  token: pathUsd,
  zoneId: ZONE_A.id,
