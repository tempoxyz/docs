...
Partners that directly call `AccountKeychain.authorizeKey(...)` to create or rotate access keys should also review the T3 upgrade page. Existing authorized access keys remain valid, but those onchain authorization flows must move to the new TIP-1011 ABI post-T3.
