...
Soon to be the default, the follower can be configured to run without trusting the RPC endpoint. In order for this to work, the endpoint must serve consensus rpcs that the follower can use to validate peered execution state. The public endpoint for the Moderato testnet supports these rpcs
...
If following against a custom endpoint, it must serve the consensus rpcs (default true for validator nodes). RPC nodes running with `--follow.experimental.certify` will serve these rpcs.
