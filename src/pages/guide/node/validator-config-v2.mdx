...
description: Manage your validator with ValidatorConfig V2. Key rotation, fee recipient separation, IP updates, and migration from V1.
---
...
- **Set a fee recipient** — route block proposal fees to a dedicated treasury address
...
Separate fee custody from validator operations by routing block proposal fees to a dedicated address:

```bash
cast send 0xCCCCCCCC00000000000000000000000000000001 \
  "setFeeRecipient(uint64,address)" \
  <YOUR_VALIDATOR_INDEX> \
...
This reduces blast radius during key compromise — an exposed operational key does not put historically collected fees at risk.
...
| **Self-service ops** | None (owner only) | IP updates, fee recipient, rotation, transfer |
| **Fee recipient** | Not supported | Separate from validator address |
...
- **Fee recipients** are stored on-chain for use in a future hardfork
