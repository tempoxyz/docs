...
If you need to reset your validator's data, you must rotate to a new validator identity.

- **Pre-T2**: Coordinate with the Tempo team to deactivate your old identity and register a new one.
- **Post-T2 (ValidatorConfig V2)**: Use the [`rotateValidator`](/guide/node/validator-config-v2#rotate-validator-identity) precompile call to self-service rotate to a new identity.
...
:::info[Pre-T2 / Post-T2]
Before the T2 hardfork, signing key rotation requires coordinating with the Tempo team. Once T2 is active and [ValidatorConfig V2](/guide/node/validator-config-v2) migration is complete, validators can rotate keys self-service using the `rotateValidator` precompile call.
:::

#### Post-T2 (ValidatorConfig V2)

Once T2 is active, use the `rotateValidator` precompile to swap to a new ed25519 key while keeping your validator index and committee slot. See the [rotation guide](/guide/node/validator-config-v2#rotate-validator-identity) for the full walkthrough.

#### Pre-T2 (ValidatorConfig V1)
...
If you lose your signing share (stored on the database in `<datadir>/consensus/`), you will need to rotate to a new validator identity.

- **Pre-T2**: Coordinate with the Tempo team to deactivate your old identity and register a new one.
- **Post-T2**: Use [`rotateValidator`](/guide/node/validator-config-v2#rotate-validator-identity) to self-service rotate to a new identity.
