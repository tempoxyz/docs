...
:::info[ValidatorConfig V1 → V2]
Tempo is migrating from ValidatorConfig V1 to [ValidatorConfig V2](/guide/node/validator-config-v2) as part of the T2 hardfork. Once T2 is active and migration is complete, V2 becomes the primary interface for managing your validator — including self-service key rotation, IP updates, and ownership transfers.

Until then, validator management operations require coordinating with the Tempo team through V1.
:::
...
If you need to reset your validator's data, you must rotate to a new validator identity. With ValidatorConfig V2 (post-T2), use [`rotateValidator`](/guide/node/validator-config-v2#rotate-validator-identity) to self-service rotate to a new identity. With V1 (pre-T2), coordinate with the Tempo team.

## Key Management (ValidatorConfig V2)

Once the T2 hardfork is active and V2 migration is complete, validators can perform the following operations self-service through the [ValidatorConfig V2 precompile](/guide/node/validator-config-v2):

### Signing Key Rotation

Use `rotateValidator` to swap to a new ed25519 key while keeping your validator index and committee slot. No coordination with the Tempo team required. See the [rotation guide](/guide/node/validator-config-v2#rotate-validator-identity) for the full walkthrough.

After rotation, your validator goes through the [standard lifecycle](#validator-states) with the new identity.

### Update IP Addresses

Use `setIpAddresses` to update your node's ingress and egress endpoints on-chain. Takes effect at the next finalized block. See the [IP update guide](/guide/node/validator-config-v2#update-ip-addresses).

### Transfer Validator Ownership

Use `transferValidatorOwnership` to rebind your validator entry to a new control address. See the [transfer guide](/guide/node/validator-config-v2#transfer-validator-ownership).
...
If you lose your signing share (stored on the database in `<datadir>/consensus/`), you will need to rotate to a new validator identity using `rotateValidator`.
...
## Legacy: Key Management (ValidatorConfig V1)

:::warning
This section applies to pre-T2 networks still using ValidatorConfig V1. Once T2 is active and V2 migration is complete, use the [V2 operations above](#key-management-validatorconfig-v2) instead.
:::

### Signing Key Rotation (V1)
...
3. Once confirmed, update your node configuration to use the new key and restart. Once the node is running, your validator will go through the [validator lifecycle](#validator-states).

:::warning
The old validator identity must be deactivated before the new one is activated
:::

### Signing Share Recovery (V1)

If you lose your signing share, coordinate with the Tempo team to deactivate your old identity and register a new one.

### Update IP Addresses (V1)
