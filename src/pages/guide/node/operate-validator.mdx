...
curl -s localhost:6060/metrics | grep consensus_engine_dkg_manager_how_often_player
...
curl -s localhost:6060/metrics | grep consensus_engine_epoch_manager_simplex_batcher_inbound_messages_total | grep data_0
```
...
curl -s localhost:6060/metrics | grep -E "marshal_finalized_height|marshal_processed_height"
```
...
Make sure your Prometheus is scraping metrics from your validator's metrics endpoint (default: `localhost:6060/metrics`).
