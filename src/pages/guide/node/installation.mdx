...
Downloading a snapshot lets your node skip syncing from genesis and start participating much faster. Validators should use `--minimal` when migrating to minimal snapshots, and should keep using `--minimal` for future validator replacements. RPC providers, indexers, and other workloads that need complete historical data should use `--archive`.
