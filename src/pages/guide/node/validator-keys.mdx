...
`--secret` accepts any filesystem path, but use a FIFO (named pipe) or shell process substitution rather than a regular file:
...
With shell process substitution:

```bash
tempo consensus generate-signing-key \
  --output <SIGNING_KEY_PATH> \
  --secret <(printf '%s' '<ENCRYPTION_KEY>')
```

Use FIFOs so the encryption key is streamed to `tempo` at the moment it is needed without putting the key in the process environment. Environment variables are visible to child processes and can be exposed through process inspection, crash dumps, shell history, or service-manager diagnostics. A FIFO keeps the secret out of argv and env, avoids writing it to disk, and lets an external secret manager provide the value just in time.
