...
```mermaid {style: {maxWidth: '600px', margin: '0 auto'}}
flowchart LR
    Start(["Add / Rotate in"]) --> A["Syncer<br/><small>Epoch E</small>"]
    A -->|"epoch boundary"| B["Player<br/><small>Epoch E+1</small>"]
    B -->|"receives share"| C["Dealer / Validator<br/><small>Epoch E+2</small>"]
    C -->|"fully synced"| D["Voter / Proposer"]
    D --> End(["Deactivate / Rotate out"])

    click A "#syncer-epoch-e"
    click B "#player-epoch-e1"
    click C "#dealer--validator-epoch-e2"
    click D "#dealer--validator-epoch-e2"
```
