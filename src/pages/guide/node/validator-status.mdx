...
```json
{
  ...
  "inboundAddress": "203.0.113.10:9000",
  "outboundAddress": "203.0.113.10:9001"
}
```

| Field | Type | Description |
|-------|------|-------------|
| `publicKey` | `bytes32` | Ed25519 consensus public key |
| `active` | `bool` | Whether the validator is active in the current contract state |
| `index` | `uint64` | Index of the validator (constant under rotation) |
| `validatorAddress` | `address` | Validator control address |
| `inboundAddress` | `string` | Inbound address (`IP:port`) for incoming connections |
| `outboundAddress` | `string` | Outbound IP address for firewall whitelisting |
