...
| Field | Description |
|-------|-------------|
| `publicKey` | Ed25519 consensus public key (`bytes32`) |
| `active` | Whether the validator is active in the current contract state |
| `index` | Index of the validator (constant under rotation) |
| `validatorAddress` | Validator control address |
| `inboundAddress` | Address where other validators connect to this validator (`IP:port`) |
| `outboundAddress` | Outbound IP address for firewall whitelisting |
