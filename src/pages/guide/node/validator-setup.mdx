...
Generate an encrypted ed25519 keypair. `--secret` points to a FIFO or another file-like path that provides the encryption key:
...
Use a FIFO instead of an environment variable so the encryption key is not stored in the process environment. Environment variables can be inherited by child processes and exposed through process inspection, crash dumps, shell history, or service-manager diagnostics. A FIFO lets `tempo` read the key once, keeps it out of argv and env, and avoids writing it to disk.
