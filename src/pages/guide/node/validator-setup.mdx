...
:::info
Use a FIFO or process-substitution path for `--secret` and `--consensus.secret`. This keeps the encryption key out of environment variables and command-line arguments, and lets `tempo` consume it once without leaving it in a regular file. See [Why FIFOs and not env vars](/guide/node/validator-keys#why-fifos-and-not-env-vars).
:::
