...
:::info[Set a token logo at creation]
The TIP-20 factory exposes a 7-arg `createToken` overload that accepts an on-chain `logoURI` at creation ([TIP-1026](https://tips.sh/1026)). Wallets and explorers can then read the icon directly from the token contract via `logoURI()`, without going through the [tokenlist registry](/quickstart/tokenlist). Per [TIP-1026's recommended formats](https://tips.sh/1026#recommended-formats), use a square, single-frame rasterized image (PNG or WebP); SVG is allowed by the scheme allowlist but not recommended. See [Optional on-chain logoURI](/protocol/upgrades/t5#optional-on-chain-logouri).
:::
