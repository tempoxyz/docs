...
:::info[Coming with T5]
After the [T5 network upgrade](/protocol/upgrades/t5), an admin holding `DEFAULT_ADMIN_ROLE` can update the on-chain `logoURI` field on any TIP-20 via `setLogoURI(string newLogoURI)` ([TIP-1026](https://github.com/tempoxyz/tempo/blob/main/tips/tip-1026.md)). Wallets and explorers can read the icon directly from the token contract via `logoURI()`. See the [token onboarding migration appendix](/quickstart/tokenlist#t4--t5-migration) for issuer guidance.
:::
