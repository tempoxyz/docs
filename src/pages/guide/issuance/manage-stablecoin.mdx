...
:::info[Update the token logo]
An admin holding `DEFAULT_ADMIN_ROLE` can set or update the on-chain `logoURI` field on any TIP-20 via `setLogoURI(string newLogoURI)` ([TIP-1026](https://tips.sh/1026)). Wallets and explorers read the icon directly from the token contract via `logoURI()`. Per [TIP-1026's recommended formats](https://tips.sh/1026#recommended-formats), use a square, single-frame rasterized image (PNG or WebP); SVG is allowed by the scheme allowlist but not recommended. See [Optional on-chain logoURI](/protocol/upgrades/t5#optional-on-chain-logouri).
:::
