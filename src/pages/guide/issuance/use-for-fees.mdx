...
To determine which validator tokens are needed, sample recent blocks and check the miner's preferred fee token using `getValidatorToken` on the FeeManager contract. For example, on Moderato testnet, roughly 80% of blocks are submitted by validators that accept fees in pathUSD, with the rest accepting AlphaUSD. On mainnet, this token mix is different and subject to change.
