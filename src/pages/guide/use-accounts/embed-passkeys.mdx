...
import { webAuthn } from 'accounts/wagmi' // [!code ++]
...
Without an `authUrl`, credentials are stored locally on the client device. This means users will lose access to their passkey account if their storage is cleared or if they sign in from another device.

For production, you should set up a [`Handler.webAuthn`](/accounts/server/handler.webAuthn) server and pass the `authUrl` to the connector:

```tsx
connectors: [webAuthn({ authUrl: 'https://myapp.com/auth' })]
```

See [Deploying to Production](/accounts/production) for more details.
