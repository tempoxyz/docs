...
After the [T6 network upgrade](/protocol/upgrades/t6), a blocked TIP-20 `transfer` / `transferFrom` still succeeds, but credits `RECEIVE_POLICY_GUARD_ADDRESS` instead of the intended receiver.
...
See [account-level receive policies on the T6 upgrade page](/protocol/upgrades/t6#account-level-receive-policies-tip-1028).
:::
