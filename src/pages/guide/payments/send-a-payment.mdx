...
:::warning[Coming with T6 — confirm delivery, not just transaction success]
After the [T6 network upgrade](/protocol/upgrades/t6), a TIP-20 `transfer` / `transferFrom` can succeed even if the intended receiver is **not credited** — when the receiver has opted in to an account-level receive policy ([TIP-1028](https://tips.sh/1028)) that blocks the inbound call. In that case the token emits `Transfer(..., RECEIVE_POLICY_GUARD_ADDRESS, amount)` and the new `ReceivePolicyGuard` precompile emits a `TransferBlocked` event with the data needed to claim the funds later.

If your app marks payments, withdrawals, or deposits complete from transaction success alone, also verify the `Transfer` recipient is the intended receiver (or its TIP-1022 master) and subscribe to `TransferBlocked` to surface redirected funds. Blocked receipts are not enumerable on chain — indexers are the only way to discover them.
:::
