...
:::info[Multihop routing]
The Fee AMM supports multihop routing ([TIP-1033](https://tips.sh/1033)) — one fee conversion can route through two pools when a direct pair between the user's fee token and the validator's payout token doesn't exist or has insufficient liquidity. Pool mechanics for adding and removing liquidity are unchanged, but a single fee conversion may reserve and consume liquidity from two pools you provide to instead of one.
:::
