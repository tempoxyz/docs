...
- **Payment optionality** — Accept both cards and stablecoins
- **Familiar UX** — Stripe's polished checkout for fiat users
- **Lower fees** — Crypto payments avoid card processing fees
- **Global reach** — Stripe for regulated markets, crypto for everywhere else
    ...
    <div>
      ...
      <div>
        {items.map((item, i) => (
          <div key={i}>
            <span>{item.name}</span>
            <span>${(item.amount / 100).toFixed(2)} × {item.quantity}</span>
          </div>
        ))}
        <div>
