            ...
            {rows.map((row, i) => (
              <Link
                ...
                className={`group flex w-full items-start justify-between gap-6 border-line border-b px-7 py-5 text-left last:border-b-0 lg:px-12 ${
                  active === i ? 'text-foreground' : 'text-white/55 hover:text-white/80'
                }`}
              >
                <span>
                  ...
                  <span className="mt-2 block max-w-[420px] font-sans text-[14px] text-white/40 leading-[1.45] tracking-[0] group-hover:text-white/50">
                    {row.desc}
                  </span>
                </span>
                <span
                  aria-hidden
                  className={`mt-1.5 size-2 shrink-0 ${active === i ? 'bg-foreground' : 'bg-white/25'}`}
                />
              </Link>
            ))}
          </div>
