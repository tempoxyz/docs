...
const CURSOR_RESPONSE_MS = 120
// #d9d9d9 — cursor and default pattern color.
const BRIGHT_RGB = '255, 255, 255'
...
export default function DotCanvas({ className, pattern, children }: Props) {
  ...
  useEffect(() => {
    ...
    // Paint a single dot (circle) at the cell's top-left pixel. fillStyle must
    // be set by the caller.
    const DOT_RADIUS = DOT_SIZE / 2.5
    const fillDot = (x: number, y: number) => {
      ctx.beginPath()
      ctx.arc(x + DOT_RADIUS, y + DOT_RADIUS, DOT_RADIUS, 0, Math.PI * 2)
      ctx.fill()
    }
    ...
    const drawCursor = () => {
      ...
      ctx.fillStyle = `rgba(${BRIGHT_RGB}, 1)`
      const { cols, rows } = gridSize()
      for (let r = 0; r < rows; r++) {
        const y = offY + r * PITCH
        if (Math.abs(y - cursor.y) > CURSOR_RADIUS) continue
        for (let c = 0; c < cols; c++) {
          const x = offX + c * PITCH
          if (Math.abs(x - cursor.x) > CURSOR_RADIUS) continue
          fillDot(x, y)
        }
      }
    }
