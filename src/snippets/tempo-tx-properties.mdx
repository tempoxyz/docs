...
<Tabs stateKey="library">
  ...
  <Tab title="Rust">
    ...
    async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Fill gas, nonce, etc. from the network
        let filled = provider.fill(tx).await?;
        let mut tempo_tx = filled.build_aa()?;

        // Fee payer signs the transaction // [!code hl]
        let fee_payer = PrivateKeySigner::from_str("0x...")?; // [!code hl]
        let sender_addr = provider.default_signer_address(); // [!code hl]
...
<Tabs stateKey="library">
  ...
  <Tab title="Go">
    ...
    func main() {
    	...
    	chainID := big.NewInt(transaction.ChainIdModerato)
