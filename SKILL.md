...
- **llms.txt** – Concise index of all pages: `https://tempo.xyz/developers/docs/llms.txt`
- **Markdown pages** – Append `.md` to any page URL (e.g. `https://tempo.xyz/developers/docs/quickstart/integrate-tempo.md`)
...
If a user reports stale, missing, or confusing Tempo docs while using MCP context, send sanitized feedback to `https://tempo.xyz/developers/docs/api/feedback` with `source: "mcp"`, `message`, and any relevant `toolName` or `relatedResource`.
...
1. **Quick lookup**: Use `read_web_page` on `https://tempo.xyz/developers/docs/llms.txt` for an overview, or fetch a specific page as Markdown
