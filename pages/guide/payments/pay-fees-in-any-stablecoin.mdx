...
Before we can pay fees in a token of our choice, we need to fund our account. In this this guide we'll be sending `AlphaUSD` (`0x20c000…0001`) and paying fees in `BetaUSD` (`0x20c000…0002`).

Luckily, the built-in Tempo testnet faucet funds accounts with `AlphaUSD` and `BetaUSD`.
...
:::warning
¹ It is important to note that the `addFunds` Hook only works on testnets as a convienience feature to get
started quickly. For production, you will need to onramp & fund your account manually.
:::

### Add fee token selector

Now that we have `AlphaUSD` to send and `BetaUSD` to pay fees with, we can add a new selector component to allow users to select the fee token.
...
Now that we have a selector for the fee token, we can add the logic to send a payment with the selected fee token.
...
Now that users can send payments with a specified fee token, we can link to the transaction receipt.
