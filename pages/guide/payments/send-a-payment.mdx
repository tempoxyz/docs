...
Enable gasless transactions by routing payments through a fee payer relay. Set up your client with the `withFeePayer` transport and use `feePayer: true` on transactions:
...
import { withFeePayer } from 'tempo.ts/viem' // [!code hl]
...
export const client = createClient({
  ...
  transport: withFeePayer( // [!code hl]
    http(), // [!code hl]
    http('https://your-fee-payer-relay.com'), // [!code hl]
...
For a complete guide on setting up fee sponsorship including running your own fee payer relay server, see the [Sponsor transaction fees](/guide/use-accounts/fee-sponsorship) guide.
