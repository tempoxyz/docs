...
Each USD TIP-20 on Tempo specifies a single other USD TIP-20 as its quote token. The quote token is the token it trades against on the [stablecoin exchange](/documentation/protocol/exchange/spec). This guarantees exactly one path between any two tokens, reducing liquidity fragmentation and simplifying routing.
...
pathUSD is a predeployed [TIP-20](/documentation/protocol/tip20/spec) at genesis. Since it is the first TIP-20 deployed, its quote token is the zero address.
...
1. Market makers provide liquidity for USDC/pathUSD and USDP/pathUSD pairs.
2. A user wants to send USDC to a merchant who prefers USDP.
3. The exchange atomically routes the payment: USDC to pathUSD to USDP.
