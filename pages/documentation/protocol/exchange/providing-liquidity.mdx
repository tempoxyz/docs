...
Provide liquidity to the exchange by placing limit orders or flip orders in the order book.
...
You can only place orders on pairs between a token and its designated quote token. All TIP-20 tokens specify a quote token for trading pairs. Most USD stablecoins use [linkingUSD](/documentation/protocol/exchange/linkingUSD) as their quote token.

## Overview

The exchange uses an on-chain order book where you can place orders at specific price ticks. Orders are matched using price-time priority, meaning better-priced orders fill first, and within the same price, earlier orders fill first.

Unlike traditional AMMs, you specify exact prices where you want to buy or sell, giving you precise control over your liquidity provision strategy.
...
## Managing DEX Balances

The exchange allows you to hold token balances directly on the exchange contract to avoid transfer costs. When placing orders, tokens are debited from your exchange balance (or transferred from your wallet if insufficient). When your orders fill, proceeds are credited to your exchange balance.

For complete details on checking balances, depositing, withdrawing, and best practices for managing your exchange balance, see the [Exchange Balance](/documentation/protocol/exchange/exchange-balance) page.
