...
LinkingUSD can also be accepted as a fee token by validators. On the Andantino testnet, all validators are configured with linkingUSD as their preferred stablecoin. Since users cannot pay fees in linkingUSD, fee payments to these validators must use the [Fee AMM].
...
As discussed [above](#transfers) of linkingUSD are subject to role-based authorization.
