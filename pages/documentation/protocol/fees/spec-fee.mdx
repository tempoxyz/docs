...
4. Validator (if validator sets a fee preference)
...
If no fee preference is set at the transaction, account, or contract level, the default fee token is the [current validator's preferred fee token](#validator-preferences). If that token is linkingUSD, the transaction is invalid (since linkingUSD cannot be paid by a user).
