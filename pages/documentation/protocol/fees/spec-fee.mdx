...
* Reserve `max_fee` of liquidity on the [fee AMM](#fee-amm) between the `fee_token` and the validator's preferred fee token. Is there is insufficient liquidity, the transaction is invalid.
...
The protocol checks for token preferences in three ways, with this order of precedence:
...
* The token must not be [linkingUSD](/documentation/protocol/exchange/linkingUSD). (While validators can accept fees in linkingUSD, users cannot pay fees in linkingUSD.)
...
If no preference is specified at the transaction, account, or contract level, the transaction is invalid.

## Transaction level

Tempo's [new transaction type](/documentation/protocol/transactions/spec-account-abstraction.mdx), allows transactions to specify a `fee_token` on the transaction. This overrides any preferences set at the account or contract level.
...
An account can specify a fee token preference for all transactions for which it is the `fee_payer` (including both transactions it sponsors as well as non-sponsored transactions for which it is the `tx.origin`). This overrides any preference set at the contract level.
