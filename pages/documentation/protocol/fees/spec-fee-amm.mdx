...
Sets the default fee token preference for the caller (user). Requires token to be a USD TIP-20 token. Cannot set linkingUSD as user's fee token. Emits `UserTokenSet` event. Access: Direct calls only (not via delegatecall).
...
Called by the protocol before transaction execution. Determines user's fee token preference (transaction → account → contract → validator). Reserves AMM liquidity if user token differs from validator token. Collects maximum possible fee from user. Returns user's token preference. Access: Protocol only (`msg.sender == address(0)`).
...
Called by the protocol after transaction execution. Calculates refund amount: `refundAmount = maxAmount - actualUsed`. Refunds unused tokens to user. Releases reserved liquidity for refunded amount. Tracks collected fees for block-end settlement. Access: Protocol only (`msg.sender == address(0)`).
...
1. **Pre-Transaction**:
   ...
   - `FeeManager.collectFeePreTx()` is called:
     - Determines user's fee token preference (transaction → account → contract → validator)
     - If user token differs from validator token, reserves AMM liquidity via `reserveLiquidity()`
     - Collects maximum fee from user using `systemTransferFrom()`
...
2. **Post-Transaction**:
   ...
   - `FeeManager.collectFeePostTx()` is called:
