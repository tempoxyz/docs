...
Let `validatorPubKey` be the public key of the validator proposing a given sub-block. Let `validatorPubKey184` be the most significant 184 bits of the validator's public key. 

We reserve sequence keys to each validator by requiring that the first (most significant) byte of the `sequence key` is the constant byte `0x5b`, and the next 23 bytes (184 bits) encode `validatorPubKey184`. 

Formally, we require that:

1. The `sequence key` of any transaction in the sub-block is of the form `(0x5b << 248) + (validatorPubKey120 << 64) + x`, where `x` is a value between `0` and `2**64 - 1`. In other words, the most significant byte of the sequence key is always `0x5b`, the next 23 bytes are the most significant 184 bits of the validator's public key, and the final 8 bytes (64 bits) still allow for 2D-nonces.

2. No two validators share the same `validatorPubKey184`; each validator's reserved space is distinct.
...
In all these scenarios, we should treat the transaction as a 'reverted' transaction: fees for the transaction are paid to the extent possible. If a 'regular' revert would have triggered a nonce increase, we should increase the nonce. 
...
In all cases, the fee is paid in the preferred `fee_token` of the `fee_recipient`, using liquidity from the fee AMM as necessary (i.e., `validatorTokens[fee_recipient]` from the FeeManager contract). If the `fee_recipient`  has not set a preferred `fee_token`, then we use LinkingUSD as a fallback. 


### 6. Sub-block Transaction Keys


We currently require that sub-block transactions are signed solely by the root EOA key of the address sending the transaction. One case that we considered is whether to allow any signature other than the root EOA key of the address sending the transaction. This would open up a transaction failure scenario: 

- The user's key was expired/ revoked earlier in the block.
 
At this point we have a few options on how to treat it:

- The proposer should not include such sub-blocks in the block: this sets up a censorship vector where a user has the power to prevent a sub-block from being included in the block.
- Include the sub-block in the block, but the transaction is a no-op: this transaction can then be included in future blocks, breaking the assumption that a given tx can appear exactly once. 
- Persist any expired/ revoked key for the block, verify that the signature was valid at the top of the block, and increment the nonce for the sub-block transaction (while still treating it as a no-op). This changes the intended semantics of a key being revoked. 

Are there other options/ objections to restricting the signature to the root EOA key?
