...
We currently allow only free transactions, i.e., transactions with `maxFeePerGas=0` in sub-blocks. Paid transactions will be allowed in a subsequent hard fork when the FeeManager is updated to handle fee accounting across sub-blocks.
</Callout>
