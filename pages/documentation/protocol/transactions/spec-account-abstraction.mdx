...
pub struct TxAA {
    ...
    aa_authorization_list: Vec<AASignedAuthorization>, // EIP-7702 style authorizations with AA signatures
}
...
0x76 || rlp([
    ...
    sender_signature         // AASignature bytes (secp256k1, P256, WebAuthn, or Keychain)
])
```
...
- The `sender_signature` field is the final field and contains the AASignature bytes (secp256k1, P256, WebAuthn, or Keychain)
...
When an AA transaction is received, the protocol:
...
2. **Validates KeyAuthorization** (if present in transaction)
   - The `key_authorization` field in `TxAA` provisions a NEW Access Key
   - Root Key MUST sign:
