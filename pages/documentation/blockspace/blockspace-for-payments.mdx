...
<Callout type="note">
    **Capacity and Priority Fees**: While the payment lane has athe compliance. much higher capacity limit than the general transaction lane, it is theoretically possible for it to fill up during extreme congestion. In such cases, transactions can set a priority fee to increase their likelihood of inclusion. Both payment and non-payment transactions share the same fixed base fee, with the ability to add priority fees when needed. The payment lane's higher capacity means congestion should be rare in practice.
</Callout>
