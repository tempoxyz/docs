...
Scheduled transactions are enabled by Tempo's [Account Abstraction transaction type](/protocol/specs/account-abstraction), which includes optional timestamp fields. These fields are validated by the protocol when the transaction is submitted.
...
For complete specifications on timestamp validation, see the [Account Abstraction](/protocol/specs/account-abstraction) transaction type specification.
