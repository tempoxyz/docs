...
Scheduled transactions are enabled by Tempo's [Account Abstraction transaction type](/protocol/transactions/account-abstraction), which includes optional timestamp fields. These fields are validated by the protocol when the transaction is submitted.
...
- **Smart contracts**: Store the signed transaction on-chain in a contract that can submit it when the time window is valid
...
For complete specifications on timestamp validation, see the [Account Abstraction](/protocol/transactions/account-abstraction) transaction type specification.
