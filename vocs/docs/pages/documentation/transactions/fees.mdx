...
<Callout type="info">
  The fee token will also be possible to configure on a transaction-by-tranasction basis via a new `fee_token` field on user originated transactions. This feature is coming soon.
</Callout>
