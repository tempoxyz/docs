...
// For a utility token, consider use case // [!code focus]
const utilityTokenCap = parseUnits('10000000', 18) // 10 million tokens // [!code focus]
await client.token.setSupplyCap({ // [!code focus]
  supplyCap: utilityTokenCap, // [!code focus]  
  token: '0x20c0000000000000000000000000000000000005', // [!code focus]
}) // [!code focus]
```
...
// If you need to reduce the cap (can't increase) // [!code focus]
const newCap = parseUnits('5000000', 18) // 5 million tokens // [!code focus]
const currentSupply = await client.token.getTotalSupply({ token }) // [!code focus]

if (newCap >= currentSupply) // [!code focus]
  await client.token.setSupplyCap({ // [!code focus]
    supplyCap: newCap, // [!code focus]
    token: '0x20c0000000000000000000000000000000000004', // [!code focus]
  }) // [!code focus]
else // [!code focus]
  console.log('Cannot set cap below current supply') // [!code focus]
```
