...
const authorizationList = [{
  chainId: chainId,
  address: '0x0000000000000000000000000000000000000000', // Zero address = undelegate
  nonce: await publicClient.getTransactionCount({ address: accountAddress }),
  ...signature // r, s, v from signing the authorization
}]

await walletClient.sendTransaction({
  authorizationList,
  to: accountAddress, // Can be any address or null
  value: 0n
})
...
const authorizationList = [{
  chainId: chainId,
  address: customImplementationAddress,
  nonce: await publicClient.getTransactionCount({ address: accountAddress }),
  ...signature // r, s, v from signing the authorization
}]
