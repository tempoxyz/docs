...
> NOTE:
> We currently allow only free transactions, i.e., transactions with `maxFeePerGas=0` in sub-blocks. Paid transactions will be allowed when the Fee Manager is updated.
...
Further, `generalGasLimit` is renamed to `mainBlockGeneralGasLimit` and is the limit for non-payment transactions in the main block (see [Payment Lane Spec](./PaymentLane.md)). 

#### 2.1 System transaction

The block includes a **new system transaction** before the stablecoin DEX system transaction, whose call data contains, for each included sub-block, the public key of the validator proposing, the `feeRecipient` for that sub-block and the signature of the sub-block. It is a no-op, and is there for execution layer blocks to be self-contained/carry all context. It must directly precede the Fee Collector system transaction (see [Fee AMM](./FeeAMM.md)).
