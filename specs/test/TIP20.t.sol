...
contract TIP20Test is Test {
    ...
    function testDepthOfLinkingUSD() public {
        // LinkingUSD should have depth 0 (base case)
        TIP20 linkingUSD = TIP20(LINKING_USD);
        assertEq(linkingUSD.depth(), 0);
    }
    ...
    function testDepthAfterChangingQuoteToken() public {
        ...
        assertEq(token.depth(), 1);
    }

    function testDepthAfterChangingToAnotherToken() public {
        // Initially token has depth 2
        assertEq(token.depth(), 2);
    ...
    function testDepthWithDeeperChain() public {
        ...
        // newToken should have depth 3 (token has depth 2, so 2 + 1 = 3)
        assertEq(newToken.depth(), 3);
    }
