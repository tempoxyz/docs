# Default Account Abstraction (DAA)
...
1) **Auto-delegation on first use.** When a top-level transaction originates from an address `A`, `nonce(A) == 0`, and `code(A)` is empty, the protocol **sets `code(A) = 0xEF0100 || DEFAULT_7702_IMPL`** during execution, thereby delegating `A` to the default implementation per 7702 semantics. Legacy tx validation is unchanged.
...
### Auto-delegation on first use

When executing a transaction with `tx.origin = A`, auto-delegation is triggered if `code(A) == EMPTY` and `nonce_pre(A) == 0`. If both preconditions are met at transaction start, the account is updated by setting `code(A) := EF_PREFIX || DEFAULT_7702_IMPL` before transaction execution begins.

If `code(A) != EMPTY` (for example, when A is a contract) or `nonce_pre(A) != 0`, the transaction proceeds without triggering auto-delegation. Accounts can later redelegate or undelegate as defined in EIP-7702 (see [Redelegation & undelegation](#redelegation--undelegation)). The gas cost of installing EF_PREFIX || DEFAULT_7702_IMPL is consistent with standard EIP-7702 delegation.
