...
<defs>
  <pattern id="mercator-benchmark-grid" width="24" height="24" patternUnits="userSpaceOnUse">
    <path d="M 24 0 H 0 V 24" stroke="#29262e" stroke-width=".45" opacity=".5"/>
  </pattern>
</defs>
<rect width="840" height="480" fill="#0b0b0b"/>
<rect width="840" height="480" fill="url(#mercator-benchmark-grid)"/>
<rect x="24" y="20" width="792" height="100" rx="3" fill="#0b0b0b" stroke="#4c436b" stroke-width=".5"/>
<rect x="94" y="150" width="666" height="200" rx="3" fill="#0b0b0b" stroke="#4c436b" stroke-width=".5"/>
<text x="40" y="44" fill="#f2f1ec" font-size="22" font-weight="600">GPT-5.6 Luna</text>
<text x="40" y="71" fill="#8b8793" font-size="11.5" font-family="Geist Mono, monospace" letter-spacing=".5">MEAN SCORE · WIDESEARCH, FRESHQA, GAIA, DEEPSYNTH</text>
<path d="M 40 104 H 76" stroke="#bcb7c3" stroke-width="1.5"/>
<text x="86" y="109" fill="#f2f1ec" font-size="12" font-family="Geist Mono, monospace" letter-spacing=".7">AGENT</text>
<path d="M 192 104 H 228" stroke="#b9a3ff" stroke-width="3" stroke-dasharray="1 7" stroke-linecap="round"/>
<text x="238" y="109" fill="#f2f1ec" font-size="12" font-family="Geist Mono, monospace" letter-spacing=".7">AGENT + MERCATOR</text>
<text font-family="Geist Mono, monospace" x="40" y="141" fill="#a7a2ad" font-size="12">SCORE (%)</text>`,
]
for (let score = 62; score <= 72; score += 2) {
  svg.push(`<path d="M 94 ${y(score)} H 760" stroke="#4c436b" stroke-opacity=".65" stroke-width=".5"/>
<text font-family="Geist Mono, monospace" x="78" y="${y(score) + 5}" text-anchor="end" fill="#a7a2ad" font-size="13">${score}</text>`)
}
svg.push(`<path d="${path('mercator')} ${path('baseline', [...points].reverse()).replace(/^M/, 'L')} Z" fill="#b9a3ff" fill-opacity=".05"/>
...
<text font-family="Geist Mono, monospace" x="${p.x}" y="${y(p.baseline) + 25}" fill="#c8c3cc" font-size="18" text-anchor="middle">${p.baseline.toFixed(1)}%</text>
<text font-family="Geist Mono, monospace" x="${p.x}" y="${y(p.mercator) - 16}" fill="#b9a3ff" font-size="18" font-weight="600" text-anchor="middle">${p.mercator.toFixed(1)}%</text>
<text font-family="Geist Mono, monospace" x="${p.x}" y="391" fill="#f2f1ec" font-size="13" text-anchor="middle">${p.effort.toUpperCase()}</text>
